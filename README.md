# Bhive - Task 
## (submitted on 09-June-2024)

A simple web app built with React JS to have an authentication system & hosting powered by Firebase, home screen

![image description](data:image/png;base64,)

![image description](https://github.com/sriesh-0204/coffee-day-task/assets/61575809/de82c1f0-415f-4b24-8ceb-212105641b17)
)

![image description](https://github.com/sriesh-0204/coffee-day-task/assets/61575809/8d8204ce-d2f6-4de2-886a-e249fd7d293b)
## Tech Stack
| Library | Version  |
| :---:   | :---: |
| React | 18.2   |
| Sass | 1.72.x   |
| Firebase | 10.9.x   |






## Installation
Assuming you're all the pre requisites installed on your machine such as NodeJS, yarn or npm package manager.

#### Clone the repo

bash
  git clone https://github.com/sriesh-0204/Bhive-Task.git
  cd Bhive-Task
  
#### Live Link: 
https://66656fb37f26cba552742179--fabulous-macaron-143115.netlify.app/


#### Install dependenciesa

bash
  yarn (or) npm i


#### Start the server locally on your machine

bash
  yarn start


## Future enhancements

- We can add the following to the project:-
    - Enhanced UI
    - Tailwind for 100% RWD 
    - Redux for state management and persist the data
    - eslint
    - Loaders & toast messages to indicate the app state
    - API data for category listing
    - folder structure and components
